...
def build_training(court_json, player_json, court_keypoints):
    rows = []
...
def postprocess_coordinates(player_tracking_filepath, court_tracking_filepath, results_dir):
    # read in court and player tracking as input
    with open(court_tracking_filepath) as court:
        court_json = json.load(court)
    with open(player_tracking_filepath) as player:
    ...
    training = build_training(court_json, player_json, court_keypoints)
    ...
    with open(os.path.join(results_dir, 'player_tracking_transformation_results.json'), 'w') as output:
